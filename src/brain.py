    ...
    def get_response(self, user_message, user_id="default_user", user_name=None, conversation_id=None):
        ...
        user_context_message = {
            ...
            "content": (
                ...
                f"\n\n[System Automation & Alarms on macOS]\n"
                f"NOVAX-AI directly connects to the user's macOS computer to schedule alarms, countdown timers, launch applications, and adjust system audio. "
                f"When an alarm is set, NOVAX-AI arms an audible background daemon that sounds a native chime (`Glass.aiff`) through their Mac speakers, displays a native desktop banner notification, adds a scheduled reminder in Apple Reminders, and opens the macOS Clock app to the Alarms tab. "
                f"The user can type 'test alarm' to test the chime audio, 'active alarms' to view scheduled alarms, or 'cancel alarm' to cancel."
                f"{style_instruction}"
                f"{prefs_text}"
                f"{memory_text}"
            )
